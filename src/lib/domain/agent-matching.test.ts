...
describe("agent matching", () => {
  it("returns Chile-capable agents for the Chile route", () => {
  ...
  it("returns China-capable agents for the China route", () => {
  ...
  it("returns South Africa-capable agents for the South Africa route", () => {
  ...
  it("excludes coverage mismatches", () => {
  ...
  it("applies issue penalties", () => {
    const matches = matchAgents({ rfqCase: chinaCase, agents: agentsSeed, coverages: coveragesSeed, metrics: metricsSeed, limit: 10 });
    const dragonGate = matches.find((match) => match.agent.id === "agent-dragon-gate");
    expect(dragonGate?.factors).toContainEqual({ label: "Recent issue flags: documentation delays", points: -20 });
  });
});
